...
[GEDCOM X Relationship Types](https://github.com/FamilySearch/gedcomx/blob/master/specifications/relationship-types-specification.md) specification.
...
publisher | A reference to the entity responsible for making the described source available. | [URI](#uri) | OPTIONAL. If provided, MUST resolve to an instance of [`http://gedcomx.org/v1/Agent`](#agent).
authors | A reference to the entities that authored the described source. | List of [URI](#uri) | OPTIONAL. If provided, MUST resolve to an instance of [`http://gedcomx.org/v1/Agent`](#agent).
...
name | value
-----|-------
`http://gedcomx.org/CharacterRegion`|A region of text in a digital document, in the form of `a,b` where `a` is the index of the start character and `b` is the index of the end character. The meaning of this qualifier is undefined if the source being referenced is not a digital document.
...
`http://gedcomx.org/PageRegion`|A single page in a multi-page document, in the form of a 1-based integer. This value always references the absolute page number, it does not reference any type of custom page number in a multi-page document. The meaning of this qualifier is undefined if the source being referenced is not a multi-page document.
`http://gedcomx.org/PageRectangleRegion`|A rectangular region of a single page in a multi-page document. The value of the qualifier is interpreted as `m:n`, where `m` is a `PageRegion` qualifier and `n` is a `RectangleRegion` qualifier. The meaning of this qualifier is undefined if the source being referenced is not a multi-page document.    
